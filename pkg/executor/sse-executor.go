...
func (s *SSEExecutor) OnWatchEvent(event Event) error {
	select {
	case s.ch <- event:
		return nil
...
func (s *SSEExecutor) Start() error {
...
func (s *SSEExecutor) Stop() error {
	return s.server.Close()
}

var _ Executor = (*SSEExecutor)(nil)
...
func NewSSEExecutor(args SSEExecutorArgs) *SSEExecutor {
	ch := make(chan Event)
	...
	return &SSEExecutor{ch: ch, server: &server, logger: logger}
}
