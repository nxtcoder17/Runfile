...
func parseDotEnv(reader io.Reader) (map[string]string, error) {
	...
	if err != nil {
		return nil, errors.ErrParseDotEnv(err)
	}
	return m, nil
}
...
func ParseDotEnvFiles(files ...string) (map[string]string, error) {
	...
	for i := range files {
		if !filepath.IsAbs(files[i]) {
			return nil, errors.ErrInvalidDotEnv(fmt.Errorf("dotenv file paths must be absolute")).KV("dotenv", files[i])
		}

		f, err := os.Open(files[i])
		if err != nil {
			return nil, errors.ErrInvalidDotEnv(err).KV("dotenv", files[i])
		}
